...
export default function EditEvent() {
    ...
    // 2. The Date Translator (Django ISO -> HTML Local)
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                // Ensure this matches your Organizer Edit URL!
                ...
                setFormData({
                    ...
                    // Safely format dates for the HTML inputs
                    start_datetime: formatForInput(data.start_datetime || data.start_time),
                ...
                // Keep track of the old image so we can display a thumbnail
                setExistingImage(data.thumbnail || data.image);
    ...
    // 4. The Submission Engine (Using PATCH for partial updates)
    const handleSubmit = async (e) => {
        ...
        // ONLY append a file if they actually selected a new one!
        if (imageFile) {
            submitData.append('thumbnail', imageFile);
        }

        try {
            // Use PATCH or PUT to update the specific ID
            await api.patch(`events/me/${id}/`, submitData, {
    ...
    if (isFetching) {
        return <div className="min-h-screen bg-gray-50 flex items-center justify-center animate-pulse text-gray-500 text-lg">Loading Event Data...</div>;
    }
        ...
        <div className="min-h-screen bg-gray-50 py-10 px-6">
            <div className="max-w-4xl mx-auto">
                <div className="mb-8">
                    ...
                    <p className="mt-1 text-sm text-gray-500">Update the details for <span className="font-semibold text-gray-700">{formData.title}</span>.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input type="text" name="short_description" value={formData.short_description} onChange={handleChange} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" maxLength="255" />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="language" value={formData.language} onChange={handleChange} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                        ...
                        {/* Notice we removed min={getMinTime()} so organizers can edit old events without date errors! */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Start Time *</label>
                                <input type="datetime-local" name="start_datetime" required value={formData.start_datetime} onChange={handleChange} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">End Time</label>
                                <input type="datetime-local" name="end_datetime" min={formData.start_datetime} value={formData.end_datetime} onChange={handleChange} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
                            </div>
                            <div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
                        ...
                        <div className="flex items-center gap-6">
                            {existingImage && !imageFile && (
                                <div className="h-24 w-36 rounded-md overflow-hidden bg-gray-100 border border-gray-200 shrink-0">
                                    <img src={existingImage} alt="Current Banner" className="h-full w-full object-cover" />
                                </div>
                            )}
                            
                            <div className="flex-1">
                                <label className="block text-sm font-medium text-gray-700 mb-1">Update Banner / Thumbnail</label>
                    ...
                    <div className="pt-4 flex justify-end sticky bottom-6 z-10 bg-white p-4 rounded-xl border border-gray-200 shadow-xl">
                        <button type="button" onClick={() => navigate('/dashboard')} className="mr-4 rounded-md border border-gray-300 bg-white px-6 py-3 text-sm font-bold text-gray-700 shadow-sm hover:bg-gray-50 transition-colors w-full sm:w-auto">
                            Cancel
                        </button>
